...
def detectWink(frame, location, ROI, cascade):
    ...
    #decrease scaleFactor if no eyes detected
    while scaleFactor > 1.001 and len(eyes) == 0:
...
def detect(frame, faceCascade, eyesCascade):
    ...
    blur_frame = cv2.GaussianBlur(dummy_frame, (3, 3), 0)
    ...
    detected = 0
    if len(faces) == 0:
        result = detectWink(frame, (0, 0), frame, eyesCascade)
        if result == 1:
            detected += 1
    ...
    for f in faces:
        ...
        h1 = int(y + h / 1.5)
        faceROI = dummy_frame[y:h1, x:x + w]
